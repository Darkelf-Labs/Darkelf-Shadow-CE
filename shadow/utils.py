...
def sanitize_url_clearurls(url: str) -> str:
    ...
    new_query = "&".join([part for part in query.split("&") if not any(part.startswith(p + "=") for p in clear_params)])
...
def apply_chromium_flags():
    ...
    os.environ["QTWEBENGINE_CHROMIUM_FLAGS"] = (
        "--disable-sync --metrics-recording-only --no-first-run --disable-breakpad disable_2d_canvas_auto_flush"
    )
