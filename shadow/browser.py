...
def force_youtube_locale(url: str) -> str:
    try:
        p = urlparse(url)
        ...
        print(f"[Darkelf] Cleanup warning: {e}")
...
class DarkelfBrowser(BrowserUIMixin, BrowserFeaturesMixin, QMainWindow):
    ...
    def on_url_entered(self):
        ...
        # Force YouTube to English / US region
        url = force_youtube_locale(url)
    ...
    def eventFilter(self, obj, event):
        ...
        if event.type() == QEvent.ShortcutOverride:
            ...
            if primary and key == Qt.Key_T:
                self._cmd_t_handled_override = True
                self.new_tab()
                event.accept()
                return True
            ...
            if primary and key == Qt.Key_W:
                self._cmd_w_handled_override = True
        ...
        if event.type() == QEvent.KeyPress:
            ...
            # New Tab
            if primary and key == Qt.Key_T:
                if self._cmd_t_handled_override:
                    # ShortcutOverride already created this tab.
                    # Consume the duplicate KeyPress only.
                    self._cmd_t_handled_override = False
                    return True
            ...
            # Close Tab
            if primary and key == Qt.Key_W:
                if self._cmd_w_handled_override:
                    # ShortcutOverride already closed the tab.
                    # Consume the duplicate KeyPress.
                    self._cmd_w_handled_override = False
                    return True
    ...
    def _add_tab(self, url=None, home=False):
        ...
        def apply_easylist_cosmetics(v=view):
            ...
            # Compatibility-mode sites already use native browser APIs.
            # Skip the large EasyList cosmetic stylesheet on these sites
            # to avoid unnecessary CSS processing and rendering overhead.
            try:
                page = v.page()
                if (
                    hasattr(page, "_compatibility_mode")
                    and page._compatibility_mode(host)
                ):
                    return
            except Exception as e:
                print(e)
        ...
        if home:
            ...
            target_url = url or "https://duckduckgo.com/lite/"
            target_url = force_youtube_locale(target_url)
