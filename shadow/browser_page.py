...
class SecondaryNavigationPage(QWebEnginePage):
    """
    Capture website-created secondary navigation.

    User-clicked links:
        Redirect into the current Shadow tab.

    Script-created popup/popunder navigation:
        Block.

    No site-specific domain exceptions are required.
    """

    def __init__(self, source_page, parent=None):
        super().__init__(source_page.profile(), parent)
    ...
    def acceptNavigationRequest(self, url, navtype, isMainFrame):
        ...
        try:
            source_url = self._source_page.url().toString()
        except Exception:
            source_url = ""

        print(
            f"[DarkelfAI] SECONDARY NAVIGATION: "
            f"{source_url} -> {destination} "
            f"type={navtype}"
        )
        ...
        # USER-CLICKED LINK
        #
        # A genuine <a target="_blank"> click arrives as
        # NavigationTypeLinkClicked.
        #
        # Keep that navigation inside the current Shadow tab.
        # ---------------------------------------------------------
        ...
        if (
            ...
            print(
                f"[DarkelfAI] USER LINK -> CURRENT TAB: "
                f"{destination}"
            )

            self._source_page.setUrl(url)
        ...
        #
        # window.open(), popunders and similar script-created
        # secondary navigation are not user link-click navigation.
        # Block them rather than allowing them to create or replace
        # a Shadow tab.
        # ---------------------------------------------------------

        print(
            f"[DarkelfAI] SCRIPT POPUP BLOCKED: "
            f"{source_url} -> {destination} "
            f"type={navtype}"
        )
...
class HardenedWebPage(QWebEnginePage):
    ...
    def javaScriptConsoleMessage(self, level, message, lineNumber, sourceID):
        ...
        Canvas decisions are automatic:
            TRUSTED   -> native canvas allowed
            PROTECTED -> Darkelf randomized/noised canvas remains active
            BLOCKED   -> native Qt canvas readback remains disabled

        No canvas permission dialog and no reload.
        """
        ...
        if isinstance(message, str) and message.startswith(challenge_marker):
            try:
                ...
                if host and self._canvas_mode(
                    ...
                    # Never reload here. Persistent challenge widgets can be
                    # rediscovered on every document and create reload loops.
                    return

            except Exception as e:
                print(
                    f"[DarkelfAI] Human verification compatibility error: {e}"
                )
        ...
        if isinstance(message, str) and message.startswith(marker):
            try:
                ...
                # Darkelf now handles canvas readback automatically.
                #
                # TRUSTED:
                #     Native canvas behavior is allowed.
                #
                # PROTECTED:
                #     Qt readback is allowed, but Darkelf's deterministic
                #     per-tab/per-domain canvas noise remains active.
                #
                # BLOCKED:
                #     Qt native canvas readback remains disabled.
                #
                # No user prompt and no page reload are required.

                if mode == "trusted":
                    print(
                        f"[DarkelfAI] Canvas readback=AUTO-ALLOWED "
                        f"for {host} ({method})"
                    )

                elif mode == "protected":
                    print(
                        f"[DarkelfAI] Canvas readback=AUTO-PROTECTED "
                        f"for {host} ({method})"
                    )

                else:
                    print(
                        f"[DarkelfAI] Canvas readback=AUTO-BLOCKED "
                        f"for {host} ({method})"
                    )

            except Exception as e:
                print(
                    f"[DarkelfAI] Automatic canvas policy error: {e}"
                )
    ...
    def createWindow(self, _type):
        """
        Capture website-created secondary navigation without creating
        a visible Shadow tab.

        The destination is redirected into the current tab.
        """

        try:
            source = self.url().toString()
        except Exception:
            source = "unknown"

        print(
            f"[DarkelfAI] SECONDARY WINDOW REQUEST: "
            f"source={source}"
        )

        page = SecondaryNavigationPage(
            self,
            self._parent_view
        )
