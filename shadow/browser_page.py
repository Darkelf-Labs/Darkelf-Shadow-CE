    ...
    def javaScriptConsoleMessage(self, level, message, lineNumber, sourceID):
        ...
        if isinstance(message, str) and message.startswith(marker):
            try:
                ...
                # Only ask when the current page is actually in BLOCKED mode.
                # Protected/trusted pages merely log their use of canvas.
                if host and self._canvas_mode(host, current_url.path()) == "blocked":
                    if host not in self._canvas_prompted_hosts:
                        self._canvas_prompted_hosts.add(host)

                        box = QMessageBox(self._parent_view)
                        box.setIcon(QMessageBox.Icon.Information)
                        box.setWindowTitle("Canvas access blocked")
                        box.setText(f"{host} requested canvas readback ({method}).")
                        box.setInformativeText(
                            "Darkelf blocked it for privacy. If a feature on this site "
                            "is not working, you can allow native canvas access for "
                            "this Darkelf session and reload the page. The permission "
                            "is cleared when Darkelf closes."
                        )
