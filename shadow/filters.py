...
class _NetRule:
    __slots__ = ("re", "is_exception", "opts", "raw")
...
class EasyListEngine:
    ...
    def _parse_network(self, line: str):
        ...
        self.network_rules.append(_NetRule(cre, is_exception, opts, raw_line))
    ...
    def should_block(
        ...
        for rule in self.network_rules:
            ...
            # ABP resource-type options must constrain a rule to the
            # corresponding request type. Without "ping" here, a rule such as
            # *$ping,third-party is incorrectly treated as a generic
            # third-party rule and can block scripts/XHR needed by websites.
            type_flags = {
                "script",
                "xmlhttprequest",
                "subdocument",
